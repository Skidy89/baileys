...
import { areJidsSameUser, BinaryNode, BinaryNodeAttributes, encodeRawMessage, getBinaryNodeChild, getBinaryNodeChildren, isJidGroup, isJidUser, jidDecode, jidEncode, jidNormalizedUser, JidWithDevice, S_WHATSAPP_NET } from '../WABinary'
		...
		await authState.keys.transaction(
			async() => {
				...
				if(isGroup || isStatus) {
					...
					const bytes = encodeRawMessage(patched)
	...
	return {
		...
		sendMessage: async(
			...
			if(
				...
				await Promise.all([
					relayMessage(jid, fullMsg.message!, { messageId: fullMsg.key.id!, useCachedGroupMetadata: options.useCachedGroupMetadata, additionalAttributes, statusJidList: options.statusJidList, additionalNodes }),
					config.emitOwnEvents && process.nextTick(() => (
